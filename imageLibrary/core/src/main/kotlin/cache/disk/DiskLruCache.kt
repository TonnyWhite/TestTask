...
import utils.deleteContents

// from https://github.com/coil-kt/coil/blob/main/coil-base/src/main/java/coil/disk/DiskLruCache.kt
...
internal class DiskLruCache(
    ...
    private fun writeJournal() = synchronized(this) {
        journalWriter?.close()
     ...
     */
    fun get(key: String): Snapshot? = synchronized(this) {
        checkNotClosed()
        validateKey(key)
        initialize()
    ...
    private fun removeEntry(entry: Entry): Boolean = synchronized(this) {
     ...
     */
    private fun launchCleanup() = synchronized(this) {
        cleanupScope.launch {
    ...
    inner class Snapshot(private val entry: Entry) : Closeable {
        ...
        override fun close() = synchronized(this@DiskLruCache) {
            if (!closed) {
                closed = true
        ...
        fun closeAndEdit(): Editor? = synchronized(this@DiskLruCache) {
            close()
    ...
    inner class Editor(val entry: Entry) : SynchronizedObject() {
         ...
         */
        private fun file(index: Int): Path = synchronized(this) {
         ...
         */
        fun commitAndGet(): Snapshot? = synchronized(this) {
            commit()
         ...
         */
        private fun complete(success: Boolean): Unit = synchronized(this) {
            check(!closed) { "editor is closed" }
            if (entry.currentEditor == this) {
